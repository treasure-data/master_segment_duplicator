                    ...
                    if isinstance(data, dict):
                        ...
                        if db_name:
                            ...
                            if table_name and not isinstance(table_name, str):
                                ...
                                return
                            refs.add((db_name, table_name) if table_name else (db_name, None))
    ...
    try:
        ...
        if os.path.exists(temp_dir):
            try:
                ...
                logger.info(f"Cleaned up temporary directory after failed clone: {temp_dir}")
...
def find_digdag_project_dir(repo_path, project_dir=None):
    ...
    if project_dir:
        ...
        if not dig_files:
            raise FileNotFoundError(f"No .dig files found in project directory: {full_path}")
    ...
    if not project_dirs:
        raise FileNotFoundError("No directories with .dig files found in the repository")
...
def create_project_archive(project_dir, project_name=None):
    ...
    logger.info(f"Created project archive for {project_name} ({len(archive_bytes)} bytes)")
    return project_name, archive_bytes

def upload_project_to_td(project_name, archive_bytes, td_api_key, region, revision=None):
    """
